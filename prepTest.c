...
int countCurseHT(HashTable ht, const char* cod) {
	...
	for (int i = 0;i < ht.dim;i++) {
		if (ht.head[i] != NULL) {
			Nod* p = ht.head[i];
			while (p) {
				if (p->info.cod && strcmp(p->info.cod,cod)==0) {
					cnt++;
				}
				p = p->next;
			}
...
//3. Funcție: salvare în altă structură
//Exemplu : salvezi toate cursele către o destinație într - o listă simplă

void listaSimplaDestinatie(HashTable ht, char * destinatie) {
	
	Nod *listaNoua=NULL;
	int hash = calculeazaHash(destinatie, ht.dim);
	Nod* p = ht.head[hash];
	while (p) {
		if (p->info.destinatie && strcmp(p->info.destinatie, destinatie) == 0) {
			adaugaCursaLSI(listaNoua, p->info);
		}
		p = p->next;
	}
	return listaNoua;
...
void stergereCurseHT(HashTable ht, char * ora) {
	for (int i = 0;i, ht.dim;i++) {
		Nod* p = ht.head[i];
		Nod* prev = NULL;
		while (p) {
			if (strcmp(p->info.oraPlecare, ora) == 0) {
				Nod* deSters = p;
				//refac legaturile:
				if (prev == NULL) //stergere din capul listei
				{
					ht.head[i] = p->next;  //mut capul listei ca va fi sters
					p = ht.head[i]; //sau p=p->next; //deplasez p-ul
				}
				else{
					prev->next = p->next;
